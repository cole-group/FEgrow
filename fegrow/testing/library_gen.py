...
flat_library = [item for sublist in library for item in sublist] #flatten list of lists
...
for i,gen_mol in enumerate(mols): #iterate over list
    tmp = tempfile.NamedTemporaryFile(suffix=".mol2") #create a temp file to read into obabel
    #print(tmp.name)
    ...
    try:
        Chem.SDWriter(smiles_to_iupac(flat_library[i])).write(back) #lol @ using a webserver to go from smiles to iupac
    except:
