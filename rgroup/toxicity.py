...
def rule_of_five(mol):
    ...
    ro5 = {"MW": MW, "HBA": HBA, "HBD": HBD, "LogP": LogP, "Pass_Ro5": pass_ro5}  # return dict of values
...
def tox_props(data):
    ...
    if isinstance(data, pd.DataFrame):
        ...
        unwanted_subs = pd.DataFrame([get_unwanted_subs(mol, substructures) for mol in mols])
        
        data = pd.concat([data, ro5, pains, unwanted_subs], axis=1)  # put results together
